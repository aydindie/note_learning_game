...
class NoteImage {
  ...
  NoteImage({
    ...
    this.offset = 0.5, //çizgi ortasında yuvarlak çizmek için
    this.color,
  });
}
...
class ClefPainter extends CustomPainter with EquatableMixin {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final clefSize = Size(50, bounds.height); // TODO : CLEF BOYUTU

    final noteHeight = bounds.height /
        _naturalPositions.length.toDouble(); //TODO : NOTA BOYUTU HERSEY SANIRIM

    final firstLineIndex = _naturalPositions
        .indexOf(clef.firstLineNotePosition); //ÇİZGİ SAYISI GİBİ
    final lastLineIndex = _naturalPositions.indexOf(clef.lastLineNotePosition);

    final firstLineIsEven = firstLineIndex % 2 == 0;

    final ovalHeight = noteHeight * 2; // TODO: YUVARLAĞIN BOYUTU
    final ovalWidth = ovalHeight * 1.5; // TODO: YUVARLAĞIN BOYUTU
    ...
    const tailHeight = 7; //TODO: KUYRUK CIZGISI UZUNLUĞU
    ...
    for (final noteImage in noteImages) {
      ...
      //TODO: notanın kısa çizgi üstüne konması
    ...
    if (_clefSymbolPainter == null || clefSize != _lastClefSize) {
      final clefSymbolScale = Platform.isAndroid
          ? (clef == Clef.Treble)
              //android treble scale\album\7CfyBI2X9cry2xrrr078OZ
              ? 1
